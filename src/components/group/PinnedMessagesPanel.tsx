...
export default function PinnedMessagesPanel({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-black/95 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-zinc-800">
        {/* Header */}
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {pinnedMessages.map((pinnedMsg) => (
                <div
                  key={pinnedMsg.id}
                  className="bg-black rounded-xl border border-zinc-800 overflow-hidden group hover:border-zinc-600 transition-colors"
                >
                  {/* Message Content */}
                  <div
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      <img
                        src={
                          ...
                          `https://ui-avatars.com/api/?name=${pinnedMsg.message.user?.displayName || 'U'}&background=random`
                        }
                    ...
                    {/* Message Text */}
                    <div className="text-zinc-300 text-sm whitespace-pre-wrap break-words pl-11">
                      ...
                      <div className="mt-3 pl-11">
                        {pinnedMsg.message.mediaType?.startsWith('image/') ? (
                          <img
                            ...
                            alt="Message media"
                            className="rounded-lg max-w-sm max-h-48 object-cover border border-zinc-800"
                          />
                        ) : pinnedMsg.message.mediaType?.startsWith('video/') ? (
                          <video
                            src={pinnedMsg.message.mediaUrl}
                            controls
                            className="rounded-lg max-w-sm max-h-48 border border-zinc-800"
                          />
                        ) : null}
                      </div>
                    )}
                          ...
                          <div key={idx} className="relative">
                            {pinnedMsg.message.mediaTypes?.[idx]?.startsWith('image/') ? (
                              <img
                                src={url}
                                alt={`Media ${idx + 1}`}
                                className="rounded-lg w-24 h-24 object-cover border border-zinc-800"
                              />
                            ) : null}
                          </div>
                        ))}
                  ...
                  <div className="flex items-center justify-between px-4 py-3 bg-zinc-900/50 border-t border-zinc-800">
                    <div className="flex items-center gap-2 text-xs text-zinc-500">
                      ...
                      <span>
                        Pinned by <span className="text-zinc-400 font-medium">{pinnedMsg.user.displayName}</span> •{' '}
                    ...
                    {canUnpin && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all disabled:opacity-50 text-xs font-medium border border-transparent hover:border-red-500/20"
                      >
